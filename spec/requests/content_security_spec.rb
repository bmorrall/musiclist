  ...
  describe "HTTP Headers" do
    context "after sign in" do
      ...
      it "includes all expected secure headers", :aggregate_failures do
        ...
        # Cookies
        session_cookie = cookies.get_cookie("_musiclist_session")
        ...
        expect(content_security_policy).to have_content("font-src 'self';")
        expect(content_security_policy).to have_content("img-src 'self' data: https:;")
        expect(content_security_policy).to have_content("object-src 'none';")
        expect(content_security_policy).to have_content("script-src 'self';")
        expect(content_security_policy).to have_content("style-src 'self';")
